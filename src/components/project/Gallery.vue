      ...
      <div class="relative w-full h-full">
        <img
          :ref="el => setImageRef(el as HTMLImageElement, index)"
              :src="image"
          class="w-full  h-[400px] object-contain object-bottom relative z-10"
          :style="{ 
            transform: `scale(${scales[index]}) translateY(${translates[index]}px)`
          }"
        />
        <div class="w-full absolute h-full inset-0 z-10 bg-gradient-to-t from-black/20  to-transparent" />

        <!-- Overlay et curseur personnalisé -->
        <div 
          v-show="variant === 'enhanced' && showOverlay && activeIndex === index"
          class="absolute inset-0 z-50 transition-opacity duration-300 pointer-events-auto"
